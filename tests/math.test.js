...
test('Should return correct addition resultant vector', () => {
    ...
    expect(v_r.x).toBe(5)
    expect(v_r.y).toBe(9)
    expect(v_r).toMatchObject({x: 5, y: 9})

    let v3 = vec2d.addVectors(v1, v2)
    expect(v3).toMatchObject({x: 5, y: 9})
})
